...
try:
    ...
    from error_handling import FAISSError, StorageError
...
class FAISSStore:
        ...
        if self.index_path.exists():
            try:
                ...
                if self.index.d != self.dimension:
                    logging.warning(f"Existing index dimension {self.index.d} doesn't match required dimension {self.dimension}")
                    logging.info(f"Creating new index with correct dimension {self.dimension}")
                    self._backup_old_index()
                    self._create_new_index()
                    return
                
                self._load_metadata()
                logging.info(f"Loaded existing FAISS index with {self.index.ntotal} vectors (dimension {self.index.d})")
    ...
    def _backup_old_index(self):
        """Backup the old index before creating a new one"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_index_path = self.index_path.parent / f"index_backup_{timestamp}.faiss"
            backup_metadata_path = self.index_path.parent / f"metadata_backup_{timestamp}.pkl"
            
            # Backup index file
            if self.index_path.exists():
                import shutil
                shutil.copy2(self.index_path, backup_index_path)
                logging.info(f"Backed up old index to {backup_index_path}")
            
            # Backup metadata file
            if self.metadata_path.exists():
                import shutil
                shutil.copy2(self.metadata_path, backup_metadata_path)
                logging.info(f"Backed up old metadata to {backup_metadata_path}")
                
        except Exception as e:
            logging.warning(f"Failed to backup old index: {e}")
