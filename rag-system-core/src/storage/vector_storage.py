...
class FAISSVectorStorage:
        ...
        def _search_sync():
            ...
            with lock:
                ...
                if search_mode == SearchMode.EXACT:
                    # For exact search, we might need more candidates for filtering
    ...
    @with_error_handling("faiss_vector_storage", "update_metadata")
    async def update_metadata(self, vector_id: str, updates: Dict[str, Any]):
        """Update metadata for a specific vector"""
        
        def _update_sync():
            # Check chunk index first
            if vector_id in self.chunk_reverse_mapping:
                faiss_idx = self.chunk_reverse_mapping[vector_id]
                if faiss_idx in self.chunk_metadata:
                    metadata = self.chunk_metadata[faiss_idx]
            ...
            # Check site index
            if vector_id in self.site_reverse_mapping:
                faiss_idx = self.site_reverse_mapping[vector_id]
                if faiss_idx in self.site_metadata:
                    metadata = self.site_metadata[faiss_idx]
        ...
        loop = asyncio.get_event_loop()
        success = await loop.run_in_executor(self.executor, _update_sync)
        
        if not success:
            raise ProcessingError(f"Vector ID {vector_id} not found")
    
    @with_error_handling("faiss_vector_storage", "delete_vectors")
    async def delete_vectors(self, vector_ids: List[str]):
        ...
        def _delete_sync():
            ...
            for vector_id in vector_ids:
                ...
                if vector_id in self.chunk_reverse_mapping:
                    faiss_idx = self.chunk_reverse_mapping[vector_id]
                    if faiss_idx in self.chunk_metadata:
                        self.chunk_metadata[faiss_idx].custom_metadata['deleted'] = True
                        self.chunk_metadata[faiss_idx].updated_at = datetime.now()
                        deleted_count += 1
                
                # Check site index
                elif vector_id in self.site_reverse_mapping:
                    faiss_idx = self.site_reverse_mapping[vector_id]
                    if faiss_idx in self.site_metadata:
                        self.site_metadata[faiss_idx].custom_metadata['deleted'] = True
                        self.site_metadata[faiss_idx].updated_at = datetime.now()
                        deleted_count += 1
        ...
        loop = asyncio.get_event_loop()
        deleted_count = await loop.run_in_executor(self.executor, _delete_sync)
        
        self.logger.info(f"Marked {deleted_count} vectors as deleted")
        
        # Clear cache to ensure deleted vectors don't appear in results
        if self.enable_caching and self.cache:
            self.cache.clear()
    ...
    async def optimize_indices(self):
        ...
        def _optimize_sync():
            ...
            # Could include index rebuilding, compression, etc.
            self.logger.info("Index optimization completed")
        
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(self.executor, _optimize_sync)
    ...
    def get_index_stats(self, vector_type: VectorType) -> IndexStats:
        ...
        if vector_type == VectorType.CHUNK_EMBEDDING and self.chunk_index:
            # Rough estimate: dimension * 4 bytes per float * number of vectors
    ...
    async def backup_indices(self, backup_path: str):
        """Create backup of indices and metadata"""
        ...
        def _backup_sync():
            # Backup indices
            if self.chunk_index:
                chunk_backup = backup_dir / "chunk_index.faiss"
                faiss.write_index(self.chunk_index, str(chunk_backup))
            
            if self.site_index:
                site_backup = backup_dir / "site_index.faiss"
                faiss.write_index(self.site_index, str(site_backup))
            
            # Backup metadata
            chunk_meta_backup = backup_dir / "chunk_metadata.pkl"
            site_meta_backup = backup_dir / "site_metadata.pkl"
            
            with open(chunk_meta_backup, 'wb') as f:
                data = {
                    'metadata': self.chunk_metadata,
                    'id_mapping': self.chunk_id_mapping,
                    'reverse_mapping': self.chunk_reverse_mapping,
            ...
            with open(site_meta_backup, 'wb') as f:
                data = {
                    'metadata': self.site_metadata,
                    'id_mapping': self.site_id_mapping,
                    'reverse_mapping': self.site_reverse_mapping,
        ...
        self.logger.info(f"Backup completed to {backup_path}")
